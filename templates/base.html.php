...
<header class="app-header">
    <div class="header-container">
        <a href="<?= $router->generatePath('') ?>" class="logo">
        ...
        <form action="/index.php" method="GET" class="search-form">
            ...
            <div class="search-pill">
                ...
                <button type="button" class="icon-btn" id="filter-btn">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            ...
            <div class="filters" id="filters">
                ...
                <div class="filter-group">
                    <?php foreach ($genres as $genre): ?>
                        <label>
                            <input type="checkbox" name="genre[]" value="<?= $genre->getId() ?>" <?= in_array($genre->getId(), $selectedGenres) ? 'checked' : '' ?>>
                            <?= $genre->getName() ?>
                        </label>
                    <?php endforeach; ?>
                </div>
                ...
                <div class="filter-group">
                    <?php foreach ($platforms as $platform): ?>
                        <label>
                            <input type="checkbox" name="platform[]" value="<?= $platform->getId() ?>" <?= in_array($platform->getId(), $selectedPlatforms) ? 'checked' : '' ?>>
                            <?= $platform->getName() ?>
                        </label>
                    <?php endforeach; ?>
                </div>
